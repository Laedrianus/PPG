...
function randomEnemy() {
  ...
  if (r < 0.7) {
    ...
    return { type:'letter', letter: MAINNET_LETTERS[Math.floor(Math.random()*MAINNET_LETTERS.length)], x: Math.random()*(GAME_WIDTH-40), y: Math.random()*-600-40, width:44, height:44 };
  }
}
...
function showBonusMessage(msg){
  ...
  setTimeout(()=>{
    if(bonusMessage.textContent===msg) bonusMessage.textContent='';
  },3000);
}
...
function startGame(){
  ...
  // Cüzdan bağlandıysa logo zaten gizlenmiş olacak; yine de güvence için:
  if (logoOverlay) logoOverlay.classList.add('hidden');
...
function applyEnhancedStyles() {
  ...
  const buttons = [ startButton, restartButton, connectWalletBtn, tweetScoreBtn, verifyTweetBtn, pharosFollowBtn, blocksenseFollowBtn ];
  buttons.forEach(button => {
    if (button) {
      button.style.cssText += buttonStyle;
      button.addEventListener('mouseenter', function() { this.style.cssText += buttonHoverStyle; });
      ...
      button.addEventListener('mousedown', function() { this.style.cssText += buttonActiveStyle; });
